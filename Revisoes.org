...
- Remover "O primeiro trabalho analisado é o" (cruft)
  - Inicia o parágrafo por Júnior et al....
  - Acertar a conjugação do verbo em seguida
- São usadas quatro dados => São usados quatro dados (gênero)
- modelo: Quan- => modelo: quan-
  - numerar as quatro com (i), (ii), (iii), (iv).
- Remover "O segundo trabalho analisado é o" (cruft)
  - Fazer os mesmos acertos listados acima
- foi usado dados => foram usados dados
- dicutido na Seção => discutido na Seção
- "e, consequentemente, neste presente trabalho," => não ficou claro
...
- Conclusão desta seção adequada. Poderia ter uma chamada para o
...
- descorre sobre => apresenta a
- descorre-se => detalha-se
...
- os dados para do INPE, => os dados do INPE,
- permanessa => permanecesse
*** Pré-processamento dos dados
- ocupando 2 Gb em mémoria. => ocupando aproximadamente 2 Gb em mémoria.
- de criar clusters locais => de instanciar um cluster computacional
  lógico sobre um cluster físico local ou remoto
*** 4.2 Implementação do método
- que implementa esse os cálculos necessário => que implementa os cálculos necessário
- a Junção Espacial é utilizado => a Junção Espacial é utilizada
- foi basicamente aplicar => consistiu em aplicar
...
- definidas em 5.1 => definidas na Equação 5.1
- ser classificados com não queimados => ser classificados como não
  queimados
- Coefiente de Dice => talvez colocar uma referência bibliográfica
