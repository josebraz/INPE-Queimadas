...
- Trocar o título "Avaliar" -> Avaliações
- Isso é ilustrado => Essa abordagem
*** Figuras 5.1 e 5.2
- se possível, colocar fundo branco para melhor diferenciar as
  tonalidades das cores
- A fonte do texto nas imagens está demasiada pequena, difícil de ler,
  aumentar para ficar pelo menos 50% do tamanho da fonte do texto do
  caption.
*** 5.2 Validação do AQ-FA
- seja computado dezenas de vezes mais rápido. => seja calculado mais rapidamente.
- que é a mesma usada no AQ30m. => sendo esta a mesma usada no AQ30m.
*** Escolha dos parâmetros do AQ-FA
- No primeiro parágrafo, se apresentam os valores de parametragem, com
  ...
  - Percebi em seguida que chegas a de fato descrever o método
    ...
    área queimada.
- de 5 métricas estatísticas => quais são as 5 métricas?
  - Dizer explicitamente quais são os nomes e que estas encontram-se
    no eixo Y.
  - No caption se escreve "seis"
- orbitas-ponto => órbitas-ponto
*** Figura 5.3
- Não ficou claro para mim o que significam os box plots. Entendo o
...
- Fundo branco.
*** Continuação
- obtidos a patir de testes. => obtidos a _partir_ de testes.
- "A função percentil apresentou os melhores resultados nas diferentes métricas"
  - Não ficou claro olhando para a Figura 5.3 e o texto do porquê percentil é o melhor.
  - Os argumentos apresentados no último parágrafo poderiam ser usados
...
- obitidos => obtidos
- A comparação é feita para as órbitas-ponto => existe alguma
  justificativa para a escolha destas órbitas-ponto em específico? Se
  sim, seria legal colocar ali.
- "que apresentou R-quadrado igual a 0.72, ou seja, 72% dos dados do
  AQ30m pode ser bem explicados pelo AQ-FA com essa resolução."
  - Tenho dúvidas sobre essa interpretação "72%", veja
    https://en.wikipedia.org/wiki/Coefficient_of_determination
  - R2 tem a ver com a variância...
- deliminata => delimitada
- a aplicação método, a direita => a aplicação do método AQ-FA, à direita,
  - a esquerda => à esquerda
- 13º90'S 48º00'O => no gráfico usa-se W ao invés de O
  - Talvez usar a mesma terminologia em inglês se for difícil
    atualizar a imagem.
- nas grandes cicatrizes => o que seria uma cicatriz?
  - dá para entender, mas como é uma terminologia ligeiramente
    diferente, seria legal explicar (aparece antes se procurares pela
    palavra).
- , acima de 3 km => km^2 ?
- O tamanho do quadrante escolhido também impacta na precisão do modelo,
