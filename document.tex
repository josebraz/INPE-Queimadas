...
Vários testes foram realizados para encontrar uma função de avaliação da área queimada (\texttt{eval(v, vs)}) que reduzisse simultaneamente os erros de omissão e comissão em diferentes regiões e períodos de tempo. Os experimentos indicaram que uma função com valores fixos, sem levar em conta a distribuição estatística dos dados, produz resultados piores. A Figura \ref{fig:comparacao_evals} apresenta os gráficos \textit{Box Plot} das métricas: Acurácia Geral (OA); Diferença de área queimada; Erros de comissão (CE); Erros de Omissão (OE); Viés (B); e Coeficiente de Dice (DC), para quatro funções de avaliação da área queimada: linear, exponencial quadrática, exponencial cúbica e percentil (conforme descritas na Seção \ref{sec:calculo_area_queimada}). Os dados foram gerados a partir da avaliação dessas funções no conjunto de órbitas-ponto e intervalo de tempo apresentados na Tabela \ref{table:regioes_validação}.
...
De forma mais detalhada, as quatro funções apresentadas têm os limiares inferiores \texttt{vmin} e superiores \texttt{vmax} iguais, equivalentes a 2.5 e 15, respectivamente. Esses valores foram obtidos a partir de experimentos comparando várias combinações. A função \texttt{percentil} teve também os parâmetros \texttt{pmax}, \texttt{vmin} assinalados como 65\% e 85\%, também obtidos a partir de testes. 

A função \texttt{percentil} apresentou os melhores resultados nas diferentes métricas e por isso é a função escolhida para todo o restante do trabalho. A função, comparada com as outras, apresentou alta Acurácia Geral (OA), baixos Erros de Omissão (OE), alto Coeficiente de Dice (DC), Viés (B) próximo a 1 e baixa diferença entre o total de área queimada. Os Erros de Comissão (CE) foram relativamente maiores, assim como na função linear, provavelmente devido a baixa resolução dos satélites provocando erros nas extremidades de uma área queimada. 

\subsection*{Análise da validação}

A Tabela \ref{table:regioes_validação} apresenta os resultados da comparação entre o AQ30m e o AQ-FA para várias órbitas-ponto, nomeadamente: 226/071, 225/069, 224/069, 222/074, 222/067, 221/074, 221/073, 221/070, 221/067, 220/074 e 220/065, abrangendo diferentes períodos de tempo. Essas regiões foram selecionadas aleatoriamente dentre as disponíveis no site do DGI para o produto AQ30m. O intervalo de tempo considerado para cada órbita-ponto é especificado na tabela, juntamente com os valores estatísticos de OA, CSI, CE, OE, B e DC, bem como a área queimada estimada pelo AQ30m e pelo AQ-FA. A tabela é ordenada pelo valor do coeficiente de Dice (DC).
...
Como parte dos objetivos deste trabalho e após a validação apresentada, foi feita a aplicação da método em todo o território nacional, partindo do ano de 2010 até o ano de 2022. Os resultados foram comparados com o AQ1km para o periodo avaliado, por ser o produto oficial do INPE para dados de área queimada de todo o Brasil.

A máquina utilizada para a computação do AQ-FA foi um computador desktop com processador Intel Core i5-9600KF Hexa-Core 3.7GHz, 32GB de memória RAM DDR4 3200MHz e um SSD NVMe de 1TB. O sistema operacional utilizado foi o Pop!\_OS, uma distribuição Linux com kernel na versão 5.16. Foram dedicados 5 processadores físicos para todo o processamento dos dados. O processamento desse período levou cerca de 30 horas no total.

Os dados de focos ativos do INPE foram divididos por meses e pelas órbitas-ponto que compreendem o território nacional. Cada órbita-ponto foi processada completamente dentro de um ano. Processar os 12 meses dentro de uma mesma órbita-ponto representou uma otimização na performance, porque foi possível manter em memória a grade correspondente sem precisar recomputar para cada mês. Esse processo foi repetido para cada ano.
