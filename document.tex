...
\begin{abstract}
Este trabalho apresenta uma metodologia capaz de estimar a área queimada a partir de focos ativos oriundos do Programa Queimadas do INPE. A metodologica consiste em transformar focos, que são pontos no espaço, em áreas de cobertura do foco, de acordo com a resolução do satélite que o detectou. O espaço é divido em quadrantes e cada quadrante é avaliado de acordo com equações que utilizam as áreas de cobertura dos focos. Por último, cada avaliação de quadrante passa por uma função de normalização, resultando na porcentagem de área queimada estimada para o quadrante. O processo apresentado recebe o nome de AQ-FA, sigla para Área Queimada por Focos Ativos. O método é parametrizável, permitindo alterar a resolução dos resultados, que neste estudo é definida como 300m. A avaliação é contra o produto AQ30m (INPE) para 20 órbitas-ponto e diferentes períodos de tempo, apresentando altos níveis de acurácia. Os resultados são obtidos a partir da aplicação do método de forma mensal para todo o território nacional, entre os anos de 2008 até 2022. Ao comparar os resultados com o AQ1km (INPE), há um indicativo de aumento na área queimada no Brasil, contrariando o conhecimento prévio. Além disso, o bioma amazônico aparece sendo o mais impactado negativamente pela diferença entre os resultados. Toda a implementação e validação é desenvolvida utilizando a linguagem Python e Jupyter Notebook, para fins de reprodutibilidade, e todo o código está disponível de forma aberta.
\end{abstract}
...
% justificativa, legado, relevancia (falar da ciencia aberta)
